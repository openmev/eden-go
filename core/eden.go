...
type Eden struct {
	...
	slotAddresses map[common.Address]bool
	slotAddressesSorted []common.Address
...
func (e *Eden) GetSlotAddress(parentStatedb *state.StateDB, headerTimestamp uint64) (map[common.Address]bool, []common.Address) {
	if parentStatedb.Root() == e.stateRoot {
		return e.slotAddresses, e.slotAddressesSorted
	}
	...
	e.stateRoot = parentStatedb.Root()
	e.slotAddresses = result
	e.slotAddressesSorted = resultSorted
