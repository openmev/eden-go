...
func (tx *Transaction) IsEdenSlotByToAddr(slotAddress map[common.Address]bool) bool {
	toAddress := tx.To()
...
func (tx *Transaction) SetWithoutGossipByUser(val bool) {
	if val {
		atomic.StoreInt32(&tx.withoutGossipByUser, 1)
	} else {
		atomic.StoreInt32(&tx.withoutGossipByUser, 0)
	}
}
func (tx *Transaction) WithoutGossipByUser() bool {
	v := atomic.LoadInt32(&tx.withoutGossipByUser)
	return v == 1
}

func (tx *Transaction) SetFromEdenRpc(val bool) {
	if val {
		atomic.StoreInt32(&tx.fromEdenRpc, 1)
	} else {
		atomic.StoreInt32(&tx.fromEdenRpc, 0)
	}
}
func (tx *Transaction) FromEdenRpc() bool {
	v := atomic.LoadInt32(&tx.fromEdenRpc)
	return v == 1
}
